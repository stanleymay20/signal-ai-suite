...
const col = (over: Partial<ColumnProfile> = {}): ColumnProfile => ({
  name: "x", position: 0, dataType: "string",
  nullable: false, uniqueRatio: 0.5, missingPercentage: 0,
...
describe("detectTimeSeries", () => {
  it("suggests the highest-unique date and the widest-range numeric", () => {
    const r = detectTimeSeries([
      ...
      col({ name: "id", dataType: "integer", uniqueRatio: 1, stats: { count: 100, missing: 0, unique: 100, min: 1, max: 100 } }),
      col({ name: "revenue", dataType: "numeric", uniqueRatio: 0.9, stats: { count: 100, missing: 0, unique: 90, min: 0, max: 5000 } }),
      col({ name: "qty", dataType: "integer", uniqueRatio: 0.5, stats: { count: 100, missing: 0, unique: 10, min: 1, max: 20 } }),
    ]);
