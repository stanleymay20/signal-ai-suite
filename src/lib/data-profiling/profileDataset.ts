...
export function profileDataset(rows: RawRow[], columns: string[]): DatasetProfile {
  ...
  const duplicateRowPercentage =
    rowCount > 0 ? Number(((duplicateRows / rowCount) * 100).toFixed(3)) : 0;

  const totalCells = rowCount * columnCount;
  const missingCells = colProfiles.reduce((a, c) => a + c.stats.missing, 0);
  const missingCellPercentage =
    totalCells > 0 ? Number(((missingCells / totalCells) * 100).toFixed(3)) : 0;

  const numericColumns = colProfiles.filter(
    (c) => c.dataType === "numeric" || c.dataType === "integer",
  ).length;
  const categoricalColumns = colProfiles.filter(
    (c) => c.dataType === "categorical" || c.dataType === "boolean",
  ).length;
  const dateColumns = colProfiles.filter(
